...
=== Structure
In general the method description and summary is extracted for all API methods.
...
* Custom `@response` tag to define HTTP response status and type
...
It is static content that will not change often so it should be defined in a static way.
...
parser:
  ...
  postProcessor: # <3>
    ...
    postProcessor2:
      key1: value1
----
<1> Output path and filename of the generate OpenAPI specification file
...
Currently the following post processors (see module link:https://github.com/chhorz/oas-generator/tree/master/oas-generator-spi[_oas-generator-spi_]) are also provided:
...
If you choose a different return object or status code during runtime you have to document these information on a place that if available at compile time.
